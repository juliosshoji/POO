...
void controller::includeSeries(){
    ...
    unique_ptr<operation> includeSeriesOp(questions, "Novo Registro", "*");
    vector<string> answers = includeSeriesOp->doUserQA();
    unique_ptr<Series> newSeries(answers[0], stoi(answers[1]), stoi(answers[2]), stoi(answers[3]), answers[4], answers[5], answers[6], stoi(answers[7]));
